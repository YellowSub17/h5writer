...
class AbstractH5Writer:
    ...
    def __init__(self, filename, chunksize, compression):
        self._f = (
            None  # Will be overwritten by implementation object with real file handle
        )
    ...
    def _initialise_tree(self, D, group_prefix="/"):
        ...
        for k in keys:
            if isinstance(D[k], dict):
                ...
                log_debug(
                    logger, self._log_prefix + "Creating group %s" % (group_prefix_new)
                )
    ...
    def _write_to_f(self, name, data):
        if data is None:
            log_warning(
                logger,
                "Data %s is None! Skipping this item as we cannot write this data type."
                % name,
            )
        elif name in self._f:
            log_warning(
                logger, "Dataset %s already exists! Overwriting with new data." % name
            )
        else:
        ...
        for k in keys:
            if isinstance(D[k], dict):
                ...
                log_debug(
                    logger,
                    self._log_prefix
                    + "Write to dataset %s at stack position %i" % (name, self._i),
                )
                if name not in self._f:
                    log_and_raise_error(
                        logger,
    ...
    def _create_dataset(self, data, name):
        ...
        try:
            ...
            log_and_raise_error(
                logger,
                self._log_prefix
                + "Could not save dataset %s. Conversion to numpy array failed"
                % (name),
            )
            return 1
        if data.nbytes == 0:
            log_and_raise_error(
                logger,
                self._log_prefix
                + "Could not save dataset %s. Dataset is empty" % (name),
            )
            return 1
        ...
        if nbytes_chunk > CHUNKSIZE_MIN_IN_BYTES:
            ...
            chunksize = int(
                numpy.ceil(float(CHUNKSIZE_MIN_IN_BYTES) / float(data.nbytes))
            )
            log_debug(
                logger,
                self._log_prefix
                + "Increase chunksize from %i to %i for dataset %s (only %i bytes for single data frame)"
                % (self._chunksize, chunksize, name, nbytes_chunk),
            )
        ...
        log_debug(
            logger,
            self._log_prefix
            + "Create dataset %s [shape=%s, chunks=%s, dtype=%s]"
            % (name, str(shape), str(chunks), str(dtype)),
        )
    ...
    def _is_stack(self, name):
        ...
        return "axes" in a.keys() and a["axes"][0].decode("utf-8").startswith(
    ...
    def _resize_stacks(self, stack_length, group_prefix="/"):
        if group_prefix == "/":
            log_info(
                logger,
                self._log_prefix + "Resize datasets to new length: %i" % stack_length,
            )
        if stack_length == 0:
            log_warning(
                logger,
                self._log_prefix
                + "Cannot resize stacks to length 0. Skip resize stacks.",
            )
            return
        ...
        log_debug(
            logger,
            self._log_prefix
            + "Resizing data sets under the following keys:"
            + str(keys),
        )
        for k in keys:
            name = group_prefix + k
            if isinstance(self._f[name], h5py.Dataset):
                if self._is_stack(name):
                    ...
                    log_debug(
                        logger,
                        self._log_prefix
                        + ("Do not resize %s because it is not a stack." % name),
                    )
            else:
    ...
    def _resize_stack(self, stack_length, name):
        ...
        log_debug(
            logger,
            self._log_prefix
            + "Resize dataset %s [old shape: %s, new shape: %s, intended new shape: %s]"
            % (name, str(old_shape), str(self._f[name].shape), str(new_shape)),
        )
